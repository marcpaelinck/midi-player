...
fetch(DATAFOLDER_URL_ABSOLUTE + "/soundfont/GAMELAN1.sf2").then(async (response) => {
    // load the soundfont into an array buffer
    let soundFontArrayBuffer = await response.arrayBuffer();

    if (response.headers.get("content-length") > 0) {
        document.getElementById("message").innerText = "SoundFont loaded.";
    } else {
    ...
    document.getElementById("play").addEventListener("click", async (event) => {
        ...
        await context.audioWorklet.addModule(new URL("../spessasynth_lib/" + WORKLET_URL_ABSOLUTE, import.meta.url)); // add the worklet
